...
function envOf(v: unknown): Environment {
  ...
  if (x === "ote" || x === "production") return x;
  throw new HttpError(
    409,
    "registrar_environment_invalid",
    "The registrar environment is invalid.",
  );
}
...
async function secret(name: string) {
  ...
  if (error || !value)
    throw new HttpError(503, "secret_missing", `${name} is not configured.`);
  return value;
}
async function decrypt(v: string | null | undefined) {
  if (!v) return "";
  const [iv, cipher] = v.split(".");
  if (!iv || !cipher)
    throw new HttpError(
      500,
      "decrypt_failed",
      "Sensitive order data is invalid.",
    );
  const master = await secret("domain_data_encryption_key"),
    kb = new Uint8Array(
      await crypto.subtle.digest("SHA-256", encoder.encode(master)),
    ),
    key = await crypto.subtle.importKey("raw", kb, "AES-GCM", false, [
      "decrypt",
    ]),
...
async function authorize(req: Request) {
  const supplied = clean(req.headers.get("x-domain-cron-secret")),
    expected = await secret("domain_internal_cron_secret");
...
async function assertNotMaintenance() {
  ...
  if (error || !data)
    throw new HttpError(
      ...
      "Domain platform configuration is missing.",
    );
  if (data.maintenance_mode)
    throw new HttpError(
      503,
      "maintenance_mode",
      "Domain operations are temporarily paused.",
    );
}
...
async function providerBalance(environment: Environment, required: number) {
  const p = await dna(environment, "/api/v1/deposit/accounts/me", "GET", null, {
      currency: "USD",
    }),
    n = Number(p.usdBalance ?? p.data?.usdBalance);
  await db
    .from("domain_provider_balance_snapshots")
    .insert({
      ...
      provider_payload: {
        usdBalance: Number.isFinite(n) ? n : null,
        tryBalance: Number.isFinite(Number(p.tryBalance))
          ? Number(p.tryBalance)
          : null,
      },
      status: Number.isFinite(n) ? "success" : "failed",
      error_message: Number.isFinite(n) ? null : "USD balance unavailable",
    });
  if (!Number.isFinite(n) || n < required)
    throw new HttpError(
      409,
      "provider_balance_too_low",
      `${environment === "ote" ? "TEST / OTE" : "LIVE"} registrar balance is insufficient for this paid order.`,
      {
        requiredUsd: required,
        availableUsd: Number.isFinite(n) ? n : null,
...
async function fetchOrder(id: string) {
  ...
  const [{ data: user }, { data: contact }, { data: domain }, { data: quote }] =
    await Promise.all([
      db.from("domain_users").select("*").eq("id", o.user_id).maybeSingle(),
      o.contact_id
        ? db
      ...
      o.provider_quote_id
        ? db
            ...
            .select("*")
            .eq("id", o.provider_quote_id)
            .maybeSingle()
        : Promise.resolve({ data: null } as any),
    ]);
  return {
    order: o as Json,
    user: user as Json | null,
    contact: contact as Json | null,
    domain: domain as Json | null,
    quote: quote as Json | null,
  };
}
function assertJobEnvironment(job: Json, environment: Environment) {
  if (envOf(job.registrar_environment) !== environment)
    throw new HttpError(
      409,
      "job_environment_mismatch",
      "The job environment does not match its parent record.",
      {
        jobEnvironment: job.registrar_environment,
        parentEnvironment: environment,
      },
    );
}
async function validatePaid(
  job: Json,
  ctx: { order: Json; quote: Json | null },
  allowProcessing = false,
) {
  const o = ctx.order,
    q = ctx.quote,
    env = envOf(o.registrar_environment);
  assertJobEnvironment(job, env);
  ...
  if (
    !q ||
    q.id !== o.provider_quote_id ||
    envOf(q.registrar_environment) !== env ||
    !q.eligible
  )
    throw new HttpError(
      409,
      "provider_quote_invalid",
      "The exact quote is missing or belongs to another environment.",
    );
  if (
    Number(q.customer_price_usd) !== Number(o.price_usd) ||
    Number(q.provider_cost_usd) !== Number(o.provider_required_cost_usd)
  )
    throw new HttpError(
      409,
      "provider_quote_mismatch",
      "The order price does not match its exact quote.",
    );
  if (ctx.domain && envOf(ctx.domain.registrar_environment) !== env)
    throw new HttpError(
      409,
      "domain_environment_mismatch",
      "The order and domain environments differ.",
    );
  return true;
}
async function markProcessing(o: Json, request: Json) {
  await db
    .from("domain_orders")
    .update({
      status: "processing",
...
async function finish(o: Json, response: Json, domainId?: string | null) {
  await db
    .from("domain_orders")
    .update({
      status: "completed",
      completed_at: now(),
      domain_id: domainId || o.domain_id || null,
...
async function notify(
  ...
  if (user)
    await db
      ...
      .insert({ user_id: user.id, type, title, message, data });
}
async function email(
  ...
  if (user)
    await db
      .from("domain_email_outbox")
      .upsert(
        {
          event_key: `${template}:${o.id}`,
          user_id: user.id,
          order_id: o.id,
          domain_id: domainId,
...
async function syncInfo(domain: Json, job?: Json) {
  const env = envOf(domain.registrar_environment);
  if (job) assertJobEnvironment(job, env);
...
async function saveDomain(values: Json) {
  const env = envOf(values.registrar_environment);
  const { data, error } = await db
    .from("domain_domains")
    .upsert(
      { ...values, registrar_environment: env, updated_at: now() },
      { onConflict: "domain_name,registrar_environment" },
    )
    .select("*")
    .single();
  if (error || !data) throw error || new Error("domain_upsert_failed");
  return data as Json;
}
async function completeRegistration(
  ...
  const fields = infoFields(info, {
      nameservers: ctx.order.nameservers,
      locked: ctx.order.lock_requested,
      privacy_enabled: ctx.order.privacy_requested,
    }),
    domain = await saveDomain({
      user_id: ctx.order.user_id,
      contact_id: ctx.order.contact_id,
      domain_name: ctx.order.domain_name,
      tld: ctx.order.tld,
      registrar_environment: env,
      registrar_order_id:
        clean(
          response.orderCode ||
            response.orderId ||
      ...
      auto_renew: true,
      metadata: {
        registrationResponse: safeProvider(response),
        lastProviderInfo: safeProvider(info),
        registrationReconciled: reconciled,
        registrarEnvironment: env,
        testMode: env === "ote",
      },
    });
  await assignRoles(domain.id, ctx.order.user_id, ctx.contact.id);
  await finish(ctx.order, response, domain.id);
  const prefix = env === "ote" ? "[TEST] " : "";
  await notify(
    ctx.user,
    "domain_ready",
    `${prefix}Domain ready`,
    `${prefix}${ctx.order.domain_name} is active.`,
    {
      domainId: domain.id,
  ...
  await email(
    ...
    domain.id,
    "domain_ready",
    `${prefix}${ctx.order.domain_name} is ready`,
    {
...
async function processRegister(job: Json) {
  const ctx = await fetchOrder(job.order_id);
  if (!(await validatePaid(job, ctx, true))) return { skipped: true };
  if (!ctx.contact)
    throw new HttpError(
      400,
      "contact_missing",
      "Registration contact is missing.",
    );
  const env = envOf(ctx.order.registrar_environment),
    isRetry =
      ctx.order.status === "processing" || Number(job.attempts || 0) > 1;
  if (isRetry) {
    try {
      const existing = await dna(env, "/api/v1/domains/info", "GET", null, {
        DomainName: ctx.order.domain_name,
      });
      return await completeRegistration(
        ...
        env,
        {
          ...
          providerInfo: safeProvider(existing),
        },
        existing,
        true,
      );
      ...
      if (!(error instanceof HttpError) || error.status !== 404)
        throw new HttpError(
          409,
          "registration_reconciliation_failed",
          "The previous registration result is still uncertain. No duplicate request was sent.",
          error instanceof HttpError ? error.details : error,
        );
    }
  }
  await providerBalance(env, Number(ctx.quote!.provider_cost_usd));
  const request = {
    domainName: ctx.order.domain_name,
    period: Number(ctx.order.years),
    nameServers: ctx.order.nameservers || [],
    isLocked: Boolean(ctx.order.lock_requested),
    privacyEnabled: Boolean(ctx.order.privacy_requested),
    contacts: contactRoles(ctx.contact),
    tldAttributes: ctx.order.tld_attributes || {},
  };
  await markProcessing(ctx.order, request);
  const response = await dna(
    env,
  ...
  if (!providerSuccess(response))
    throw new HttpError(
      502,
      "registration_failed",
      "The registrar did not confirm registration.",
      response,
    );
  let info: Json = response;
  try {
    info = await dna(env, "/api/v1/domains/info", "GET", null, {
      DomainName: ctx.order.domain_name,
    });
  } catch {}
  return await completeRegistration(ctx, env, response, info, false);
}
async function processTransfer(job: Json) {
  const ctx = await fetchOrder(job.order_id);
  if (!(await validatePaid(job, ctx))) return { skipped: true };
  if (!ctx.contact)
    throw new HttpError(400, "contact_missing", "Transfer contact is missing.");
  const env = envOf(ctx.order.registrar_environment),
    code = await decrypt(ctx.order.auth_code_ciphertext);
  if (code.length < 4)
    throw new HttpError(
      400,
      "auth_code_missing",
      "Transfer authorization code is missing.",
    );
  await providerBalance(env, Number(ctx.quote!.provider_cost_usd));
  const check = await dna(env, "/api/v1/domains/transfers/check", "POST", {
      domainName: ctx.order.domain_name,
      authCode: code,
    }),
    transferable = bool(
      check.transferAvailabilityStatus ??
        check.data?.transferAvailabilityStatus,
    );
  if (!transferable)
    throw new HttpError(
      409,
      "transfer_not_available",
      clean(check.message || check.data?.message) ||
  ...
  await markProcessing(ctx.order, { ...request, authCode: "[encrypted]" });
  const response = await dna(env, "/api/v1/domains/transfer", "POST", request);
  if (!providerSuccess(response))
    throw new HttpError(
      502,
      "transfer_failed",
      "The registrar did not accept the transfer.",
      response,
    );
  const domain = await saveDomain({
    ...
    registrar_environment: env,
    registrar_order_id:
      clean(response.orderCode || response.orderId || response.id) || null,
    ...
    nameservers: ctx.order.nameservers || [],
    metadata: {
      transferResponse: safeProvider(response),
      transferCheck: safeProvider(check),
      registrarEnvironment: env,
      testMode: env === "ote",
    },
    last_synced_at: now(),
  ...
  await notify(
    ctx.user,
    "transfer_started",
    `${prefix}Transfer started`,
    `${prefix}Transfer request for ${ctx.order.domain_name} was submitted.`,
    {
      domainId: domain.id,
      orderId: ctx.order.id,
      registrarEnvironment: env,
...
async function processRenew(job: Json) {
  const ctx = await fetchOrder(job.order_id);
  if (!(await validatePaid(job, ctx))) return { skipped: true };
  if (!ctx.domain)
    throw new HttpError(404, "domain_not_found", "Renewal domain not found.");
  const env = envOf(ctx.order.registrar_environment);
  await providerBalance(env, Number(ctx.quote!.provider_cost_usd));
  const request = {
    domainName: ctx.order.domain_name,
  ...
  if (!providerSuccess(response))
    throw new HttpError(
      502,
      "renewal_failed",
      "The registrar did not confirm renewal.",
      response,
    );
  let updated = ctx.domain;
  try {
    updated = await syncInfo(ctx.domain, job);
  } catch {
    const base =
      ctx.domain.expires_at &&
      new Date(ctx.domain.expires_at).getTime() > Date.now()
        ? new Date(ctx.domain.expires_at)
        : new Date();
    base.setUTCFullYear(base.getUTCFullYear() + Number(ctx.order.years));
    await db
      .from("domain_domains")
      .update({
        status: "active",
        ...
        metadata: {
          ...(ctx.domain.metadata || {}),
          renewalResponse: safeProvider(response),
        },
        updated_at: now(),
      })
      .eq("id", ctx.domain.id);
  ...
  await finish(ctx.order, response, ctx.domain.id);
  const prefix = env === "ote" ? "[TEST] " : "";
  await notify(
    ctx.user,
    "renewal_completed",
    `${prefix}Renewal completed`,
    `${prefix}${ctx.order.domain_name} was renewed.`,
    {
...
async function processRestore(job: Json) {
  ...
  if (!(await validatePaid(job, ctx))) return { skipped: true };
  if (!ctx.domain)
    throw new HttpError(404, "domain_not_found", "Restore domain not found.");
  const env = envOf(ctx.order.registrar_environment);
  await providerBalance(env, Number(ctx.quote!.provider_cost_usd));
  const request = { domainName: ctx.order.domain_name };
  await markProcessing(ctx.order, request);
  const response = await dna(env, "/api/v1/domains/restore", "POST", request);
  if (!providerSuccess(response))
    throw new HttpError(
      502,
      "restore_failed",
      "The registrar did not accept the restore request.",
      response,
    );
  await db
    .from("domain_domains")
    .update({
      ...
      metadata: {
        ...(ctx.domain.metadata || {}),
        restoreResponse: safeProvider(response),
      },
      next_sync_at: new Date(Date.now() + 3600_000).toISOString(),
      updated_at: now(),
    })
...
async function processSync(job: Json) {
  ...
  return await syncInfo(domain as Json, job);
}
async function processNameservers(job: Json) {
  ...
  const env = envOf(domain.registrar_environment);
  assertJobEnvironment(job, env);
  const ns = Array.isArray(job.payload?.nameServers)
    ? [...new Set(job.payload.nameServers.map(clean).filter(Boolean))]
    : [];
  if (ns.length < 2 || ns.length > 13)
    throw new HttpError(
      400,
      "invalid_nameservers",
      "Provide between 2 and 13 nameservers.",
    );
  const response = await dna(env, "/api/v1/domains/dns/name-server", "PUT", {
    domainName: domain.domain_name,
    nameServers: ns,
  });
  await db
    ...
    .update({
      ...
      metadata: {
        ...(domain.metadata || {}),
        nameserverResponse: safeProvider(response),
      },
      last_synced_at: now(),
      updated_at: now(),
    })
...
async function processJob(job: Json) {
  if (job.type === "register_domain") return processRegister(job);
  if (job.type === "transfer_domain") return processTransfer(job);
  if (job.type === "renew_domain") return processRenew(job);
  if (job.type === "restore_domain") return processRestore(job);
  if (job.type === "sync_domain") return processSync(job);
  if (job.type === "update_nameservers") return processNameservers(job);
  throw new HttpError(
    400,
    "unknown_job",
    `Unknown domain job type: ${job.type}`,
  );
}
async function run(limit = 20) {
  ...
  let completed = 0,
    failed = 0,
    dead = 0;
  const environments: { ote: number; production: number } = {
    ote: 0,
    production: 0,
  };
  for (const job of jobs || []) {
    try {
      const env = envOf(job.registrar_environment);
      environments[env]++;
      ...
      if (job.order_id && isDead) {
        await db
          ...
          .update({
            status: "failed",
            failure_code:
              e instanceof HttpError ? e.code : "provider_operation_failed",
            failure_message: msg,
            updated_at: now(),
          })
          .eq("id", job.order_id);
  ...
  return { claimed: jobs?.length || 0, completed, failed, dead, environments };
}
Deno.serve(async (req) => {
  try {
    if (req.method === "GET")
      return json({
        ok: true,
        service: "KmerHosting Domain Jobs",
        version: 4,
    ...
    return json({
      success: true,
      result: await run(Math.max(1, Math.min(50, Number(body.limit || 20)))),
      at: now(),
    });
  } catch (e) {

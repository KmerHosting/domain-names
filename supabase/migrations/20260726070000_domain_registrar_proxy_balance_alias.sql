...
begin
  ...
  if p_path = '/api/v1/deposit/accounts/me' and v_payload ? 'usdBalance' and not (v_payload ? 'balance') then
    v_payload := v_payload || jsonb_build_object('balance', v_payload->'usdBalance');
  end if;
  if p_path = '/api/v1/deposit/accounts/me' and v_payload ? 'tryBalance' and not (v_payload ? 'availableBalance') then
    v_payload := v_payload || jsonb_build_object('availableBalance', v_payload->'tryBalance');
  end if;

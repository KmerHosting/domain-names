...
function PublicHeader() {
  ...
  return <header className="public-header"><div className="container header-row">
    <Brand />
    <button className="icon-button mobile-only" onClick={() => setOpen((value) => !value)} aria-label="Toggle navigation">{open ? <X /> : <Menu />}</button>
    <nav className={open ? "public-nav open" : "public-nav"}>
      <a href="#search">Search</a><a href="#pricing">Pricing</a><a href="#features">Features</a><Link to="/transfer-domain">Transfer</Link>
      {session ? <Link to="/dashboard" className="button button-primary">Dashboard</Link> : <><Link to="/auth" search={{}} className="button button-ghost">Sign in</Link><Link to="/auth" search={{ mode: "register" }} className="button button-primary">Create account</Link></>}
    </nav>
  </div></header>;
}
...
function PublicFooter() {
  return <footer className="footer"><div className="container footer-grid">
    ...
    <div><strong>Platform</strong><Link to="/transfer-domain">Transfer a domain</Link><Link to="/auth" search={{}}>Customer sign in</Link><a href="mailto:support@kmerhosting.com">Technical support</a></div>
...
function AuthPage() {
  ...
  return <div className="auth-page"><div className="auth-shell"><Link to="/"><Brand /></Link><div className="auth-card"><div className="auth-tabs"><button className={mode === "login" ? "active" : ""} onClick={() => { setMode("login"); setStep("form"); }}>Sign in</button><button className={mode === "register" ? "active" : ""} onClick={() => { setMode("register"); setStep("form"); }}>Create account</button></div><h1>{mode === "login" ? "Sign in" : mode === "register" ? step === "form" ? "Create your account" : "Verify your email" : step === "form" ? "Reset your password" : "Enter the verification code"}</h1><p>{mode === "login" ? "Use your email and password." : step === "form" ? "A six-digit code will be sent by email." : `Enter the code sent to ${email}.`}</p>
    <form className="form-stack" onSubmit={submit}>
      {step === "form" && <><label>Email<input name="email" type="email" required defaultValue={email} /></label>{mode === "register" && <><label>Full name<input name="fullName" required /></label><div className="form-row"><label>Phone<input name="phone" /></label><label>Country code<input name="countryCode" maxLength={2} placeholder="CM" /></label></div></>}<label>Password<input name="password" type="password" minLength={10} required /></label></>}
      {step === "otp" && <><input type="hidden" name="email" value={email} /><label>Six-digit code<input name="code" inputMode="numeric" pattern="[0-9]{6}" required /></label>{mode === "reset" && <label>New password<input name="newPassword" type="password" minLength={10} required /></label>}</>}
      <button className="button button-primary" disabled={busy}>{busy ? <LoaderCircle className="spin" size={18} /> : mode === "login" ? "Sign in" : step === "form" ? "Send code" : "Verify"}</button>
    </form>{error && <div className="alert alert-error">{errorText(error)}</div>}{mode === "login" && <button className="text-button" onClick={() => { setMode("reset"); setStep("form"); }}>Forgot password?</button>}{step === "otp" && <button className="text-button" onClick={() => setStep("form")}>Use a different email</button>}
  </div></div></div>;
}
...
function PurchasePage({ type }: { type: "registration" | "transfer" }) {
  ...
  if (!session) return <><PublicHeader /><main className="section"><div className="container narrow"><EmptyState icon={<KeyRound />} title="Sign in required" text="Sign in before creating a domain order." action={<Link to="/auth" search={{}} className="button button-primary">Sign in</Link>} /></div></main><PublicFooter /></>;
  return <><PublicHeader /><main className="section"><div className="container purchase-layout"><div><span className="kicker">{type === "registration" ? "Register domain" : "Transfer domain"}</span><h1>{type === "registration" ? "Create a registration order" : "Transfer a domain to KmerHosting"}</h1><p>Creating an order does not debit the provider. Payment is made separately from your USD account balance.</p></div>
    <section className="card"><form className="form-stack" onSubmit={check}><label>Domain name<input value={domainName} onChange={(event) => { setDomainName(event.target.value); availability.reset(); }} placeholder="example.com" required /></label><button className="button button-secondary" disabled={availability.isPending}>{availability.isPending ? "Checking…" : type === "registration" ? "Check availability and pricing" : "Load transfer pricing"}</button></form>{availability.isError && <div className="alert alert-error">{errorText(availability.error)}</div>}{result && <div className={isAvailable(result.registrar) || type === "transfer" ? "alert alert-success" : "alert alert-error"}>{type === "registration" ? isAvailable(result.registrar) ? `${result.domainName} is available.` : `${result.domainName} is not available.` : `Transfer pricing loaded for ${result.domainName}.`} {premiumDisplayPrice(result) !== null && <strong>{formatMoney(premiumDisplayPrice(result) || 0)}</strong>}</div>}
      {(type === "transfer" || result && isAvailable(result.registrar) && selectedPrice) && <div className="form-stack"><label>WHOIS contact<select value={contactId} onChange={(event) => setContactId(event.target.value)} required><option value="">Select a contact</option>{(contacts.data?.contacts || []).map((contact) => <option key={contact.id} value={contact.id}>{contactName(contact)} · {contact.email}</option>)}</select></label>{!contacts.data?.contacts.length && <div className="alert alert-warning">Create a complete WHOIS contact first. <Link to="/dashboard/contacts">Open contacts</Link>.</div>}<label>Period<select value={years} onChange={(event) => setYears(Number(event.target.value))}>{periods.map((period) => <option key={period} value={period}>{period} year{period === 1 ? "" : "s"}</option>)}</select></label>{type === "transfer" && <label>EPP/auth code<input value={authCode} onChange={(event) => setAuthCode(event.target.value)} minLength={4} maxLength={35} required /></label>}<label><input type="checkbox" checked={customNameservers} onChange={(event) => setCustomNameservers(event.target.checked)} /> Use custom nameservers</label>{customNameservers && <div className="form-stack">{nameservers.map((value, index) => <div className="dns-add-row" key={index}><input value={value} onChange={(event) => setNameservers(nameservers.map((item, position) => position === index ? event.target.value : item))} placeholder={`ns${index + 1}.example.com`} required /><button type="button" className="button button-secondary" disabled={nameservers.length <= 2} onClick={() => setNameservers(nameservers.filter((_, position) => position !== index))}>Remove</button></div>)}<button type="button" className="button button-secondary" disabled={nameservers.length >= 13} onClick={() => setNameservers([...nameservers, ""])}>Add nameserver</button></div>}<AttributeFields definitions={type === "registration" ? selectedPrice?.provider_attributes || [] : []} values={attributes} onChange={setAttributes} /><button className="button button-primary" disabled={createOrder.isPending || !contactId}>{createOrder.isPending ? "Creating exact quote…" : "Create wallet order"}</button>{createOrder.isError && <div className="alert alert-error">{errorText(createOrder.error)}</div>}</div>}
    </section>
...
function DashboardLayout() {
  ...
  return <div className="dashboard-shell"><aside className={open ? "dashboard-sidebar open" : "dashboard-sidebar"}><div className="sidebar-brand"><Brand /></div><nav className="sidebar-nav"><Link to="/dashboard" activeOptions={{ exact: true }}><LayoutDashboard size={18} />Overview</Link><Link to="/dashboard/domains"><Globe2 size={18} />Domains</Link><Link to="/dashboard/orders"><FileText size={18} />Orders</Link><Link to="/dashboard/wallet"><WalletCards size={18} />Wallet</Link><Link to="/dashboard/contacts"><UserRound size={18} />Contacts</Link><Link to="/dashboard/invoices"><FileText size={18} />Invoices</Link><Link to="/dashboard/profile"><Settings2 size={18} />Profile</Link><a href="/dashboard/notifications"><Bell size={18} />Notifications</a>{me.data?.user.role === "admin" && <a href="/admin"><ShieldCheck size={18} />Administration</a>}</nav><button className="sidebar-logout" onClick={() => logout.mutate()}><LogOut size={18} />Sign out</button></aside><main className="dashboard-main"><header className="dashboard-header"><button className="icon-button mobile-only" onClick={() => setOpen((value) => !value)}><Menu /></button><div /><div className="header-account"><span>{me.data?.user.fullName || "Customer"}</span><small>{me.data?.user.email}</small></div></header><Outlet /></main></div>;
}

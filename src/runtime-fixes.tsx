...
type WalletOrder = {
  ...
  created_at: string;
  domain_payments?: Array<{ status: string }>;
};
...
function WalletWidget() {
  ...
  useEffect(() => { if (hasSession) void loadSummary(); }, [hasSession]);
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    setOpen(true);
    walletApi<{ topup: Record<string, any> }>("/topups/check", { method: "POST", body: { topupId: walletTopupId } })
      .then((result) => {
  ...
  async function payWithBalance(orderId: string) {
    ...
    try {
      ...
      setMessage("Order paid from balance. We are now processing your domain order.");
      await loadSummary();
  ...
  return <>
    <button className="khd-wallet-launcher" onClick={() => setOpen(true)}><span>Balance</span><strong>{formatMoney(summary?.balanceUsd || 0)}</strong></button>
    {open && <div className="khd-wallet-backdrop">
      <aside className="khd-wallet-panel" aria-label="Rechargeable balance">
        <div className="khd-wallet-header"><div><small>Rechargeable balance</small><h2>{formatMoney(summary?.balanceUsd || 0)}</h2></div><button onClick={() => setOpen(false)} aria-label="Close wallet">×</button></div>
        ...
        <section className="khd-wallet-section"><h3>Recharge balance</h3><form onSubmit={submitTopup} className="khd-wallet-form"><label>Amount in USD<input type="number" min="5" max="5000" step="1" value={topupAmount} onChange={(event) => setTopupAmount(event.target.value)} /></label><label>Payment phone<input value={topupPhone} onChange={(event) => setTopupPhone(event.target.value)} placeholder="2376…" /></label><label>Method<select value={topupMethod} onChange={(event) => setTopupMethod(event.target.value)}><option value="orange_money">Orange Money</option><option value="mtn_momo">MTN MoMo</option><option value="stripe">Card</option><option value="paypal">PayPal</option></select></label><button disabled={loading}>Recharge balance</button></form></section>
        <section className="khd-wallet-section"><h3>Pay orders with balance</h3>{payableOrders.length ? payableOrders.map((order) => { const enough = Number(summary?.balanceUsd || 0) >= Number(order.price_usd || 0); return <div className="khd-wallet-order" key={order.id}><div><strong>{order.domain_name}</strong><small>{order.order_number} · {order.type} · {formatDate(order.created_at)}</small></div><div><span>{formatMoney(order.price_usd)}</span><button disabled={!enough || busyOrderId === order.id} onClick={() => payWithBalance(order.id)}>{busyOrderId === order.id ? "Paying…" : enough ? "Pay with balance" : "Insufficient"}</button></div></div>; }) : <p className="khd-wallet-empty">No pending order can be paid from balance.</p>}</section>
        <section className="khd-wallet-section"><h3>Recent transactions</h3>{summary?.transactions?.length ? summary.transactions.slice(0, 5).map((tx) => <div className="khd-wallet-tx" key={tx.id}><span>{String(tx.transaction_type).replaceAll("_", " ")}</span><strong>{formatMoney(tx.amount_usd)}</strong></div>) : <p className="khd-wallet-empty">No wallet transaction yet.</p>}</section>
      </aside>
    </div>}
  </>;
}
